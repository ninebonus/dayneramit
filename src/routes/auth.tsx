...
function AuthPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (mode === "signup") {
        ...
        toast.success("สมัครสำเร็จ กรุณาเช็คอีเมลเพื่อยืนยัน");
      } else {
  ...
  async function handleGoogle() {
    ...
    if (result.error) {
      toast.error(result.error.message ?? "เข้าสู่ระบบด้วย Google ล้มเหลว");
      setLoading(false);
      return;
    }
    ...
    <div className="min-h-screen bg-background flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md rounded-2xl border border-border bg-card p-8 shadow-soft">
        ...
        <h1 className="mt-4 text-2xl font-bold text-foreground">
          {mode === "signin" ? "เข้าสู่ระบบเอกสาร" : "สมัครสมาชิก"}
        </h1>
        <p className="mt-1 text-sm text-muted-foreground">
          ระบบออกใบเสนอราคา/ใบแจ้งหนี้/ใบเสร็จ Day Neramit
        </p>

        <button
          onClick={handleGoogle}
          disabled={loading}
          className="mt-6 w-full rounded-xl border border-border bg-background px-4 py-3 text-sm font-medium hover:bg-accent disabled:opacity-50 flex items-center justify-center gap-2"
        >
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-4">
          ...
          >
            {loading ? "กำลังดำเนินการ..." : mode === "signin" ? "เข้าสู่ระบบ" : "สมัคร"}
          </button>
        </form>
